...
# <bitbar.version>v0.2.1</bitbar.version>
...
def status
  begin
    @status ||= JSON.parse URI("https://status.heroku.com/api/v3/current-status").read
  rescue => e
...
def issues
  @issues ||= status["issues"].collect do |issue|
      title = issue["title"]
      color = issue["status_prod"]
      href = issue["full_url"]
      "#{title} | color=#{color} href=#{href}"
  end
end
...
puts "---"
puts "All good. Relax | href=https://status.heroku.com/ color=green" if issues.size == 0
puts issues.join "\n"
